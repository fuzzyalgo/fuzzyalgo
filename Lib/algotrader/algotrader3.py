...
class Algotrader3():
    ...
    def __init__(self, account= None, usePid = False, useScalp=False):
        ...
        self.cf_accounts = {}
        with open('cf_accounts.json', 'r') as f: self.cf_accounts = json.load(f)
        ...
        self.cf_symbols_default = 'GBPJPY'
        ...
        log_filename = "../logs/" + self.dt_start_str + ".log"
        format_str = '%(asctime)s: %(message)s'
        format_str = '%(message)s'
        logging.basicConfig(filename=log_filename, level=logging.DEBUG, format=format_str)
    ...
    # END def get_ticks_and_rates(sym): 
    # =============================================================================
    ...
    def mt5_fx_scalper(self, sym, vol, piddelta = None ):
        ...
        orders = self.mt5_cnt_orders_and_positions( sym )
        ...
        #  'order_pos_buy': 0,
        #  'order_pos_buy_vol': 0,
        #  'order_pos_buy_price': 0,
        #  'order_pos_sell': 0,
        #  'order_pos_sell_vol': 0,
        #  'order_pos_sell_price': 0}        
        orders = self.mt5_cnt_orders_and_positions( sym )
        ...
        if ( 1      == orders['order_pos_buy'] ) :
            ...
            if ( 2      == orders['total'] ) and \
               ...
               ( (2*vol)== orders['order_pend_sell_vol'] ) :
                ...
                priceBPO = orders['order_pos_buy_price']
        ...
        if ( 1      == orders['order_pos_sell'] ) :
            ...
            if ( 2      == orders['total'] ) and \
               ...
               ( (2*vol)== orders['order_pend_buy_vol'] ) :
                ...
                priceSPO = orders['order_pos_sell_price']
    ...
    # =============================================================================
        ...
        ret = False
        for order in orders:
            if (self.mt5.ORDER_TYPE_BUY_STOP == order.type) or (self.mt5.ORDER_TYPE_SELL_STOP == order.type):
                request = {
    ...
    # =============================================================================
        ...
        return dict( \
                ...
                order_pos_buy_price   = float(order_pos_buy_price), \
                order_pos_sell        = int(  order_pos_sell), \
                order_pos_sell_vol    = float(order_pos_sell_vol), \
                order_pos_sell_price  = float(order_pos_sell_price), \
             )
    ...
    # =============================================================================
        ...
        if (0 != resultBPO.retcode) : 
            ...
            print(" XXX ERROR PENDING ORDERCHECK FAILED ") 
            print("")
            # TODO raise error here
            return
            ...
            if pos.type == self.mt5.ORDER_TYPE_BUY or pos.type == self.mt5.ORDER_TYPE_SELL:
                ...
                for tries in range(10):
                    ...
                    if pos.type == self.mt5.ORDER_TYPE_BUY:
                        r = self.mt5_position_raw_order(self.mt5.ORDER_TYPE_SELL, symbol, pos.volume, info.bid, comment, pos.ticket)
                    else:
                        r = self.mt5_position_raw_order(self.mt5.ORDER_TYPE_BUY, symbol, pos.volume, info.ask, comment, pos.ticket)
                    # check results
                    if r is None:
                        return None
    ...
    # =============================================================================
        ...
        if price is not None:
            return self.mt5_position_raw_order(self.mt5.ORDER_TYPE_BUY, symbol, volume, price, comment, ticket)
        # no price, we try several times with current price
        for tries in range(10):
            info = self.mt5.symbol_info_tick(symbol)
            r = self.mt5_position_raw_order(self.mt5.ORDER_TYPE_BUY, symbol, volume, info.ask, comment, ticket)
            if r is None:
                return None
            if r.retcode != self.mt5.TRADE_RETCODE_REQUOTE and r.retcode != self.mt5.TRADE_RETCODE_PRICE_OFF:
                break
    ...
    # =============================================================================
        ...
        # with direct call
        if price is not None:
            return self.mt5_position_raw_order(self.mt5.ORDER_TYPE_SELL, symbol, volume, price, comment, ticket)
        # no price, we try several times with current price
        for tries in range(10):
            info = self.mt5.symbol_info_tick(symbol)
            r = self.mt5_position_raw_order(self.mt5.ORDER_TYPE_SELL, symbol, volume, info.bid, comment, ticket)
            if r is None:
                return None
            if r.retcode != self.mt5.TRADE_RETCODE_REQUOTE and r.retcode != self.mt5.TRADE_RETCODE_PRICE_OFF:
                break
    ...
    # =============================================================================
    #  def run_symbol( self, symbol, dt_from = None, mplus = None, minus = None, dfpcmax = None, mplus_change = None, verbose = None, gdt_from = None  ):
    #     
    # =============================================================================
    def run_symbol( self, symbol, dt_from = None, dt_to = None, mplus = None, minus = None, dfpcmax = None, mplus_change = None, verbose = None, gdt_from = None  ):
    # =============================================================================
        
    
        ana_key_str = self.assemble_ana_key( gdt_from, mplus, minus, dfpcmax, mplus_change )
        ...
        if 0 < len_dfana_prev:
            ...
            npa[0]['mplus'] = self.mplus
            npa[0]['minus'] = self.minus
