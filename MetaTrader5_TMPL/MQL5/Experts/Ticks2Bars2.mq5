...
void add(datetime time = 0)
{
    ...
    if(SymbolInfoTick(_Symbol, t))
    {
        ...
        if(apply(time, t, r[0]))
        {
            ...
            ResetLastError();
            /*r[0].time = iTime(_Symbol, PERIOD_M1, 0);
            r[0].close = 1.6;
            r[0].open  = 1.3;
            r[0].high  = 2.2;
            r[0].low   = 0.8;*/
...
void _OnTick(void)
{
    ...
    ResetAllTickBarsIfStopAllOrNewDay( stopAll );
...
//|                                                                  |
//+------------------------------------------------------------------+
void ResetAllTickBarsIfStopAllOrNewDay( bool stop_all = false )
{
    ...
    if( false == stop_all )
    {
        ...
        if( dt0.day_of_week != dt1.day_of_week )
        {
            ...
            if( str_apx != symbolNameAppendix )
            {
                ...
                // TODO error handling
                int ret = _OnInit();
                Print( "newDay: ",  stopAll, " _OnInit: ", ret );
            }
