...
//---
input bool InpMarketWatch = false;
//---
#include <Trade\SymbolInfo.mqh>
#include <ChartObjects\ChartObjectsTxtControls.mqh>
//---
#include "Examples\SymbolInfo\SymbolInfoSampleInit.mqh"
//+------------------------------------------------------------------+
...
//---
    ...
    int sym_custom_cnt = 0;
    int sym_custom_cnt_deselected = 0;
    int sym_custom_cnt_deleted = 0;
//--- delete custom symbols
    for(i = 0; i < m_num_symbols; i++)
    {
        string symbol = SymbolName(i, InpMarketWatch);
        bool is_custom = SymbolInfoInteger( symbol, SYMBOL_CUSTOM );
        if( true == is_custom )
        {
            sym_custom_cnt++;
            if( SymbolSelect( symbol, false ) )
            {
                sym_custom_cnt_deselected++;
                Print( "custom symbol deselected: ", symbol);
            }
            else
            {
                Print( "symbol: " , symbol, " deselect error: " , GetLastError());
            }
            if( CustomSymbolDelete( symbol) )
            {
                sym_custom_cnt_deleted++;
                Print( "custom symbol deleted: ", symbol);
            }
            else
            {
                Print( "symbol: " , symbol, " delete error: " , GetLastError());
            }
        }        
    } // for(i = 0; i < m_num_symbols; i++)
    Print( "custom symbols: ", sym_custom_cnt, " - deselected: ",  sym_custom_cnt_deselected, " - deleted: ",  sym_custom_cnt_deleted);
