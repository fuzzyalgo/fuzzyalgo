...
cPeriodArray = [28800,14400,7200,3600,1800,900,450,225,120,60,30,15]
cPeriodArray = [      14400,7200,3600,1800,900,450,225,120,60,30,15]
cPeriodArray = [                 3600,1800,900,450,225,120,60,30,15]

cPeriodArray = [28800,14400,7200                                   ]
cPeriodArray = [                 3600,1800,900                     ]
cPeriodArray = [                               450,225,120         ]
cPeriodArray = [                                           60,30,15]

cPeriodArray = [28800,14400,7200,3600,1800,900,450,225,120,60,30,15]

cPeriodArray = [                 3600,1800,900                     ]
cPeriodArray = [                               450,225,120         ]
cPeriodArray = [7200,3600,1800,900,450,225,120,60,30,15]
...
class MT5(Terminal):
    ...
    def __init__(self, account = None, symbol = None ):
        ...
        # store created config json file _cf_fn
        self.cf_config['ARGS']['PERIODS'] = cPeriodArray
...
# END def get_fuzzy_membership( data, data_step = 0.1 ):
# =============================================================================
...
# =============================================================================
    ...
    gNpa['B_pricedf']  = 'pricedf'
    for n in _periodArray:
        #gNpa['priced_'+str(n)]  = np.round(((pd.Series(gNpa['price']) - (pd.Series(gNpa['price']).rolling( n ).max() + pd.Series(gNpa['price']).rolling( n ).min())/2))/_point, decimals=0)
        # TODO next line ist not working, why? workaroung is to pass in _priced
        #gNpa['pricedf_'+str(n)]  = get_fuzzy_membership( (gNpa['priced_'+str(n)]))
    ...
    #
    # https://numpy.org/doc/stable/reference/arrays.datetime.html
    tnow = np.array(_timeObj.gDtTo_epoch_ms).astype('datetime64[ms]')
    topen=gNpa['time_msc'][0].astype('datetime64[ms]')
    tclose=gNpa['time_msc'][-1].astype('datetime64[ms]')
    d_s = int((  gNpa['time_msc'][0] - gNpa['time_msc'][-1])/1000)
    if 0 < _verbose: 
...
def test():
    ...
    # get ticks - cPeriodArray.max()
    _npa1 = gTime.update_ticks(cSym)
    _npa3 = gH.mt5_get_ticks( cSym, cPeriodArray  )
    ...
    _npa4 = gH.mt5_get_ticks( cSym, cPeriodArray, True )
    ...
    print( _headerStr )
    print()
    _npa1_out = Term_MainFunction( cSym, cPeriodArray, _npa1, gTime)
    _npa2_out = Term_MainFunction( cSym, cPeriodArray, _npa2, gTime)
    print()
    print( _headerStr )
    print()
    ...
    print( _headerStr )
    print()
    _npa3_out = MainFunction( cSym, cPeriodArray, _npa3, gH)
    _npa4_out = MainFunction( cSym, cPeriodArray, _npa4, gH)
    print()
    print( _headerStr )
    print()
    ...
    print( 'from: ', _o.gDtFrom, '  to: ', _o.gDtTo )
    _npa2 = gH.mt5_get_ticks( cSym, cPeriodArray  )
    print("len: ",len(_npa1), " cmp OK: ", ((_npa1==_npa2).all()), _npa1)
