...
def main():
    ...
    _periodArray_43 = [                                           100]
    _periodArray   = [_periodArray_21,_periodArray_32]
    ...
    for cSym in gConf.cf_config['ARGS']['SYMBOLS']:
        ...
        for _npa in _npaa:
            ...
            _npa = np.flip(_npa)
            #_npa = _npa[40000:65000]
            t = np.arange(0, len(_npa), 1)

            if 0 == cnt:
                #y20 = np.ones(len(_npa))*20
                #plt.plot(t, y20, label='y20', linewidth=0.5)
                y0 = np.zeros(len(_npa))
                plt.plot(t, y0, label=None, linewidth=0.5)
                #plt.plot(t, y0, label='y0', linewidth=0.5)
            cnt = cnt+1
            ...
            if 1000 == _cnt_median:
                key = 'cnt_d_s_median'
                plt.plot(t, (-100+_npa[key]*20), label=key+'_'+str(_cnt_median), linewidth=0.5)
                key = 'd_s_median'
                #plt.plot(t, (_npa[key]/10), label=key+'_'+str(_cnt_median), linewidth=0.5)

            if 1000 < _cnt_median:
                key = 'priced_median'
                plt.plot(t, _npa[key]/10,  linewidth=0.1)
                key = 'pricedf_median'
                plt.plot(t, _npa[key], linewidth=0.1)
